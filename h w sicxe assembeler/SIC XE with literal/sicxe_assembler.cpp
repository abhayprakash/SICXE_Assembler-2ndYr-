        ...
        if(word[2]!="")//the opcode will be in word[1]//label is there
          {
              ...
              if(word[2][0]=='=')
              {
                  string temp=word[2];
                  temp=temp.substr(1);
                  if(search_littab(temp)!=-1)
                    continue;
                  else
                  {
                      lit_tab[lit_count].name=temp;
                      if(temp[0]=='C')
                      {
                          string sval=temp.substr(2,temp.length()-3);
                          lit_tab[lit_count].value=0;
                          for(int k=0;k<temp.length();k++)
                            {
                                int n_char=temp[k];
                                lit_tab[lit_count].value=lit_tab[lit_count].value*16*16+n_char;
                            }
                          lit_tab[lit_count].length=temp.length();
                          //locctr+=lit_tab[lit_count].length;
                      }
                      else if(temp[0]=='X')
                      {
                          ...
                          int n;
                          inp>>hex>>n;
                          lit_tab[lit_count].value=n;
                          lit_tab[lit_count].length=ceil((float)temp.length()/2.0);
                          //locctr+=lit_tab[lit_count].length;
                      }
                      lit_count++;
                  }
          ...
          else// label is not there : just increment the locctr
          {
              ...
              if(word[0]=="LTORG")
              {
                  for(int i=0;i<lit_count;i++)
                  {
                    lit_tab[i].adress=locctr;
                    record[rec_count].Label="*";
                    record[rec_count].Loc=locctr;
                    record[rec_count].Operator="="+lit_tab[i].name;
                    locctr+=lit_tab[i].length;
                  }
                  lit_count=0;
                  rec_count++;
                  continue;
