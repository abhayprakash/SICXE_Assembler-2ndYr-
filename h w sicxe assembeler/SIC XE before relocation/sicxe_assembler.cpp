...
/*************************************** Pass 2 from here ***************************************/
    ...
    for(int i=0;i<rec_count;)
    {
        if(record[i].Operator=="END")
          break;
        while(record[i].ObjectCode==0&&record[i].Operator!="LDA")
          i++;
        start_here=i;
        upto_here=i;
        while(record[upto_here].Loc-record[start_here].Loc<=30)
          {
              upto_here++;
              if(record[upto_here].ObjectCode==0&&record[upto_here].Operator!="LDA")
                break;
          }
    ...
    outfile<<"E^"<<hex<<setw(6)<<setfill('0')<<start_adr;
    cin.get();
}
